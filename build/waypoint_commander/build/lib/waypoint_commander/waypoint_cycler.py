...
class FrontierDetector(Node):
    ...
    # 选择目标
            if self.callback_first:
                ...
                self.get_logger().info("First farthest frontier detected")
            else:
                target = self.locate_nearest_frontier(frontiers, origin_x, origin_y, resolution)
                self.get_logger().info("Closest frontier detected")
    ...
    def bt_log_callback(self, msg):
        for event in msg.event_log:
            # ✅ 监听两个关键节点
            if event.node_name in ('NavigateToPose', 'NavigateRecovery'):
                status = event.current_status
                self.get_logger().info(f"{event.node_name} status: {status}")

                # ✅ 只要任务完成（成功/空闲/结束），都标记 idle
                if status in ('SUCCEEDED', 'SUCCESS', 'SUCCEED', 'IDLE'):
                    self.is_idle = True
                    self.current_waypoint = None
                    self.get_logger().info("Reached goal → Ready for next frontier")

                # 🚶 如果正在执行，无论是导航还是恢复，都标记为 busy
